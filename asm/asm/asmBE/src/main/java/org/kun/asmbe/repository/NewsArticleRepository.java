...
public interface NewsArticleRepository extends JpaRepository<NewsArticle, Integer> {
    ...
    @Query("""
            ...
            AND (
                :keyword IS NULL
                OR :keyword = ''
                OR LOWER(n.newsTitle) LIKE LOWER(CONCAT('%', :keyword, '%'))
                OR LOWER(n.headline) LIKE LOWER(CONCAT('%', :keyword, '%'))
                OR LOWER(n.newsContent) LIKE LOWER(CONCAT('%', :keyword, '%'))
            )
        """)
    ...
    """)
        List<NewsArticle> searchNewsByCreator(
